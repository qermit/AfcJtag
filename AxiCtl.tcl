...
namespace eval IPCore::common {
	...
	tcl::OptProc ReadRegister {
		...
		set debug [dict get ${params} debug]
		variable addr_width
		variable data_width
		...
		set register_val 0
		if { ${debug} } {
			puts "current offset R0: ${current_offset} => ????????"
		}
		if { ! ${dry} } {
		    # do not reset hw_axi - get some starange behavior
			#reset_hw_axi [get_hw_axis ${transport}]
	
			set tmp_hw_axi [create_hw_axi_txn HW_AXI_TMP_RO [get_hw_axis ${transport}]  -force -address ${current_offset} -len 1 -size ${addr_width} -type read]
		...
		if { ${debug} } {
			puts "current offset R0: ${current_offset} => ${register_val}"
		}
		return ${register_val}
	}
	...
	tcl::OptProc WriteRegister {
		...
		if { ${debug} } {
			puts "current offset WR: ${current_offset} <= ${current_data}"
		}
		if { ! ${dry} } {
			#reset_hw_axi [get_hw_axis ${transport}]

			set tmp_hw_axi [create_hw_axi_txn HW_AXI_TMP_WR [get_hw_axis ${transport}]  -force -address ${current_offset} -data ${current_data} -len 1 -size ${addr_width} -type write]
			#set tmp_hw_axi HW_AXI_TMP
...
namespace eval IPCore::Xilinx::IIC {
	...
	tcl::OptProc reset {
        ...
        IPCore::common::WriteRegister -params ${params} [dict get ${REGS} CR] 0x2
		IPCore::common::WriteRegister -params ${params} [dict get ${REGS} CR] 0x1
		
		## czy to jest potrzebne? - tutaj chyba problem
		IPCore::common::WriteRegister -params ${params} [dict get ${REGS} THDDAT] 0x100
		# enable global interrupt
		IPCore::common::WriteRegister -params ${params} [dict get ${REGS} GIE] 0x80000000
	}
	...
	tcl::OptProc write_read {
		...
		set debug [dict get ${params} debug]
		#if {  ${debug}  } {
		#	puts [format "IIC::write_read %s | %s -> %s | rd_len: %d" ${params} ${chip_addr} ${data_write} ${data_to_read_len} ]
		#}
		...
		if { [ llength ${data_write} ] > 0 } {
			IPCore::common::WriteRegister -params ${params} ${REG_TX_FIFO} [ expr ${iic_addr_w} + 0x100 ]
			set isr_status [ isr_get -params ${params} ]
			puts "ISR_STATUS ${isr_status}"
			for { set i 0 } { ${i} < [ llength ${data_write} ] } { incr i } {
				...
				IPCore::common::WriteRegister -params ${params} ${REG_TX_FIFO} ${data_to_write}
				set isr_status [ isr_get -params ${params} ]
				puts "ISR_STATUS ${isr_status}"
			}
		}
		...
		if ${data_to_read_len} {
			return [iic_read  -params ${params} -chip_addr ${chip_addr} ${data_to_read_len}]
		}
	}


	tcl::OptProc iic_read {
		{-params -dict {} "Params"}
		{-transport -string "hw_axi_1" "Transport interface"}
		{-bus_offset -string IIC_OFFSET	"AXI ip core address"}
		{-chip_addr -strint 0x00 "Chip address"}
		...
		for {set i 0} { ${i} < ${data_to_read_len} } {incr i } {
			IPCore::common::WriteRegister -params ${params} ${REG_TX_FIFO} [ expr ${iic_addr_r} + 0x100 ]
			set isr_status [ isr_get -params ${params} ]
			puts "ISR_STATUS ${isr_status}"
			#IPCore::common::WriteRegister -params ${params} ${REG_TX_FIFO} [ expr ${len} + 0x200 ]
			lappend lista [IPCore::common::ReadRegister -params ${params} ${REG_RX_FIFO} x1]
		}
		return ${lista}
	...
	tcl::OptProc detect_all {
		{-params -dict {} "Params"}
		{-transport -string "hw_axi_1" "Transport interface"}
		{-bus_offset -string IIC_OFFSET	"AXI ip core address"}
		{?start? -int 8 "Start id"}
		{?stop? -int 120 "stop id"}
	} {
		dict for {param key} {-transport transport -bus_offset bus_offset} {
			if { [ lsearch ${Args} ${param} ] >= 0 } {
				dict set params ${key} [set ${key}]
		...
		for { set i ${start} }  { ${i} < ${stop} } {incr i } {
			...
			set addr [ expr ${i} * 2 ]
				IPCore::common::WriteRegister -params ${params} 0x120 0xF
				IPCore::common::WriteRegister -params ${params} 0x100 0x2
				IPCore::common::WriteRegister -params ${params} 0x100 0x1
				
			isr_clear -params ${params}
			IPCore::common::sleep 50
			IPCore::common::WriteRegister -params ${params} 0x108 [ expr ${addr} + 0x300 ]
			IPCore::common::sleep 50
			
			set isr_status [ isr_get -params ${params} ]
			#puts [format "Dupa isr status: %d"  [string index ${isr_status} 6] ]
	...
	tcl::OptProc isr_get {
		...
		variable REG_ISR
		return [ IPCore::common::ReadRegister  -params ${params} ${REG_ISR} b1 ] 
		
	}

	tcl::OptProc isr_print {
		{-params -dict {} "Params"}
	} {
	...
	tcl::OptProc isr_toggle {
		{-params -dict {} "Params"}
		{val "Value"}
	} {
		variable REG_ISR
		IPCore::common::WriteRegister -params ${params} ${REG_ISR} ${val}
	}

	tcl::OptProc isr_clear {
		{-params -dict {} "Params"}
	} {
		set isr_status [ isr_get -params ${params} ]
		isr_toggle -params ${params} "0b${isr_status}" 
	}
...
namespace eval IPCore::Xilinx::SPI {
	...
	tcl::OptProc SlaveSelect {
		...
		{?assert? -boolean False "Assert/deassert"}
	} {
...
namespace eval IPCore::Xilinx::GPIO {
	...
	tcl::OptProc  setVal {
		...
		IPCore::common::WriteRegister -params ${params} -bus ${bus_offset} ${reg} ${value}
	...
	tcl::OptProc  getVal {
		...
		#update_cached_out ${bus_offset} ${gpio_id} ${value}
		return [ IPCore::common::ReadRegister ${bus_offset} ${reg} ]
...
namespace eval Bus::IC::ISLA216P {
	...
	tcl::OptProc reset_soft {
		...
		IPCore::common::sleep 1000
		puts stdout [ format "Test: %s" [ IPCore::Xilinx::SPI::wr -params ${params} -chip ${chip_select} -reg 0x00 -read 1 ] ]
	}
	...
	tcl::OptProc init_1 {
		...
		#// default LVDS 3mA, two'offset binary
		IPCore::Xilinx::SPI::wr -params ${params} -chip ${chip_select} -reg 0x73 -data { 0x24 }
	...
	tcl::OptProc setTestMode {
	    ...
	    {-bus_offset -string 0x44A00000	"AXI ip core address"}
		{-chip_select -int 0 "Chips select" }
		{-test_mode -choice {None User Max Min Mid Ramp} }
		{-pattern -list {} }
	} {
		dict for {param key} {-transport transport -bus_offset bus_offset} {
			if { [ lsearch ${Args} ${param} ] >= 0 } {
		...
		switch ${test_mode} {
			...
			User { 
				...
				for {set i 0} { ${i}  < [ llength ${pattern} ] } {incr i} {
					...
					IPCore::Xilinx::SPI::wr -bus ${bus_offset} -chip ${chip_select} -reg [expr 0xC1 + ${i} * 4 ] -data ${tmp_lsb}
					IPCore::Xilinx::SPI::wr -bus ${bus_offset} -chip ${chip_select} -reg [expr 0xC1 + ${i} * 4 + 1 ] -data ${tmp_msb}
				}
...
namespace eval Bus::IC::SI57x {
	...
	tcl::OptProc read_initial {		
		{-params -dict {} "Params"}
		{-debug  -boolean False "Debug mode"}
		{-transport -string "hw_axi_1" "Transport interface"}
		{-bus_offset -string 0x40800000	"AXI ip core address"}
		{-chip_addr -strint 0x92 "Chip address"}
	} {
	
		dict for {param key} {-transport transport -bus_offset bus_offset -debug debug} {
			if { [ lsearch ${Args} ${param} ] >= 0 } {
				dict set params ${key} [set ${key}]
			} elseif { [ dict exists ${params} ${key} ] } {
				set ${key} [dict get ${params} ${key}]
		...
		IPCore::Xilinx::IIC::write_read -params ${params} -chip ${chip_addr} { 0x87 0x01 }
		set initial_array [IPCore::Xilinx::IIC::write_read -params ${params} -chip ${chip_addr} {0x7} 12 ]
	...
	tcl::OptProc set_new_freq {		
		...
		IPCore::Xilinx::IIC::write_read -params ${params} -chip ${chip_addr} { 0x89 0x10 }   
		IPCore::Xilinx::IIC::write_read -params ${params} -chip ${chip_addr} { 0x87 0x30 }
		...
		IPCore::Xilinx::IIC::write_read -params ${params} -chip ${chip_addr} [concat { 0x7 } $new_regs ]
		IPCore::Xilinx::IIC::write_read -params ${params} -chip ${chip_addr} { 0x89 0x00 }
		IPCore::Xilinx::IIC::write_read -params ${params} -chip ${chip_addr} { 0x87 0x40 }
...
namespace eval Bus::BPM {
	...
	variable SPI_ADC_OFFSET 0x44A00000
	# fmc1_board/spi_FMC1_pll
	variable SPI_PLL_OFFSET  0x44A20000

	variable FMC_SELECTIO_CTL {0x40010000 0x40030000 0x40030000}
    proc get_default_offset { name } {
			variable IIC_OFFSET
			variable SPI_ADC_OFFSET
			variable SPI_PLL_OFFSET
			variable FMC_SELECTIO_CTL
			
			if { ${name} == IIC_OFFSET } { return ${IIC_OFFSET} }
			if { ${name} == SPI_ADC_OFFSET } { return ${SPI_ADC_OFFSET} }
			if { ${name} == SPI_PLL_OFFSET } { return ${SPI_PLL_OFFSET} }
			if { ${name} == FMC_SELECTIO_CTL } { return ${FMC_SELECTIO_CTL} }
			
	}
	proc EnableVCXO {} {
		#puts IPCore::Xilinx::IIC::setGPO -params ${params} -bus ${IIC_OFFSET} 0b00000110
		variable IIC_OFFSET
		set gpo_state [IPCore::Xilinx::IIC::getGPO -bus ${IIC_OFFSET}]
		puts [format "TEST: %s" ${gpo_state}]
		set gpo_state [ expr { ${gpo_state} | 0b0111 } ]
		
		IPCore::Xilinx::IIC::setGPO -params {debug True} -bus ${IIC_OFFSET} [ format "0x%x" ${gpo_state}]
	}
	
	proc FMC1_ADC250_init {} {
		set params [dict create dry False debug False]
		variable IIC_OFFSET
		variable SPI_PLL_OFFSET
		variable SPI_ADC_OFFSET
		variable FMC_SELECTIO_CTL 
	...
	#VCXO PD L ?
	IPCore::Xilinx::IIC::setGPO -params ${params} -bus ${IIC_OFFSET} 0b00000110
	IPCore::common::sleep 500
	...
	IPCore::Xilinx::IIC::setGPO  -params ${params} -bus_offset ${IIC_OFFSET} 0b00000111
	IPCore::common::sleep 500
	Bus::IC::AD9510::init_1 -params ${params} -bus_offset ${SPI_PLL_OFFSET}
	set AD9510_status [ IPCore::Xilinx::SPI::wr -bus_offset ${SPI_PLL_OFFSET} -read_length 1 ]
	IPCore::common::sleep 500
	#reset ADC HW
	...
	IPCore::Xilinx::IIC::setGPO -params ${params} -bus_offset ${IIC_OFFSET} 0b00000111
	IPCore::common::sleep 500
	set ADC_XOR {0x00EE0000 0x00530000 0x00000000 0x00000000}
	
	for {set i 0} { ${i} < 4 } { incr i } {
		Bus::IC::ISLA216P::reset_soft -params ${params} -bus ${SPI_ADC_OFFSET} -chip ${i}
	}
	#return
	
	for {set i 0} { ${i} < 2 } { incr i } {
		
		Bus::IC::ISLA216P::init_1 -params ${params} -bus ${SPI_ADC_OFFSET} -chip ${i}
        
		# set XOR - not required anymore
		#IPCore::Xilinx::GPIO::setVal -params ${params} -bus_offset [ lindex ${FMC_SELECTIO_CTL} ${i} ] 0 [ lindex ${ADC_XOR} ${i} ]
		
	## test mode - check bis polarity
	#Bus::IC::ISLA216P::setTestMode -params ${params} -bus  ${SPI_ADC_OFFSET} -chip 0 -test_mode User -pattern { 0xFFFF }
	#Bus::IC::ISLA216P::setTestMode -bus  ${SPI_ADC_OFFSET} -chip 0 -test_mode User -pattern { 0x0001 }
	## test mode - io aligment
	  Bus::IC::ISLA216P::setTestMode -params ${params} -bus  ${SPI_ADC_OFFSET} -chip  ${i} -test_mode User -pattern { 0x0000 }
	  puts stdout [ IPCore::Xilinx::SPI::wr -params ${params} -bus ${SPI_ADC_OFFSET} -chip ${i} -reg 0xC0 -read 1 ]

	
	## test mode - line aligment -> delay
		#Bus::IC::ISLA216P::setTestMode -params ${params} -bus  ${SPI_ADC_OFFSET} -chip  ${i} -test_mode Ramp
	}
...
proc test1 {values} {
	IPCore::Xilinx::GPIO::setVal  -bus_offset 0x40030000 0 0x00530000
	...
	#IPCore::Xilinx::GPIO::setVal  -bus_offset 0x40020000 0 0xFFFFFFFF
	
	IPCore::Xilinx::GPIO::setVal  -bus_offset 0x40030000 0 0x01530000
	IPCore::Xilinx::GPIO::setVal  -bus_offset 0x40030000 0 0x00530000
}
